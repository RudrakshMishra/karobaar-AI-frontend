...
import { RefreshCw, TrendingUp, AlertTriangle, BrainCircuit, Loader2 } from "lucide-react";
...
export default function OverviewPage() {
  ...
  const { suggestions, isLoadingSuggestions, fetchSuggestions, generateSuggestions, isGenerating } = useAiStore();
  ...
  const displayProducts = products.length > 0 ? products.slice(0, 4).map((p, i) => ({
    rank: i + 1,
    name: p.name,
    realProfit: Math.round((p.current_price - p.cost) * (p.current_stock || 10)),
    margin: p.margin_percent || 50
  })) : [
    { rank: 1, name: "Noise Cancelling Earbuds PRO", realProfit: 210000, margin: 60 },
    { rank: 2, name: "Ergonomic Office Chair X1", realProfit: 155000, margin: 47 },
    { rank: 3, name: "Smart Fitness Watch", realProfit: 98000, margin: 60 },
    { rank: 4, name: "Mechanic Keyboard RGB", realProfit: 72000, margin: 48 },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#FFFFFF] border border-[rgba(212,163,115,0.18)] rounded-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={generateSuggestions} disabled={isGenerating} className="text-xs flex items-center gap-2 text-[rgba(0,0,0,0.6)] hover:text-[#050505] transition-colors disabled:opacity-50">
              <RefreshCw className={`w-3 h-3 ${isGenerating ? "animate-spin" : ""}`} /> {isGenerating ? "Generating..." : "Generate AI Insights"}
            </button>
          </div>

          <div className="space-y-0 divide-y divide-[#422D22]">
            {suggestions.length === 0 && !isLoadingSuggestions && !isGenerating ? (
              <div className="py-8 text-center text-[rgba(0,0,0,0.3)]">No insights available. Click generate to analyze your store.</div>
              ...
              suggestions.slice(0, 3).map((insight: any, idx: number) => (
                <div key={insight.id || idx} className="py-4 flex items-start gap-4 hover:bg-[rgba(0,0,0,0.02)] transition-colors px-2 -mx-2 rounded-lg">
                  ...
                  <div className="flex-1">
                    <span className="text-[10px] uppercase tracking-wider text-[rgba(0,0,0,0.4)] font-bold">{insight.type || insight.category}</span>
                    <p className="text-[#050505] text-sm font-medium mt-1 mb-2">{insight.title || insight.description}</p>
                    <div className="flex items-center gap-3">
                      <button className="text-xs bg-[#1A1A1A] text-white font-bold px-3 py-1 rounded hover:opacity-80">
                        {insight.action || "Review"}
                      </button>
                      <span className="text-xs text-[#050505] font-medium">{insight.priority === 'high' ? 'High Impact' : 'Medium Impact'}</span>
                    </div>
                  </div>
                </div>
              ))
